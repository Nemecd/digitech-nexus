...
import { Fingerprint, Building2, GraduationCap, BookOpen, MonitorPlay, Plane } from "lucide-react";
...
const categoryMeta: Record<string, { label: string; icon: any }> = {
  ...
  student_placement: { label: "Overseas Student Placement", icon: GraduationCap },
...
export default async function ShopPage() {
  ...
  const categories = Object.keys(categoryMeta).filter((key) => grouped[key]?.length);
      ...
      <section className="relative bg-navy">
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-16 pb-12">
          ...
          <p className="text-cream/70 text-lg mt-4 max-w-xl">
            Services, courses, and eBooks — add what you need to your cart and check out in one go.
          </p>
        </div>
      </section>
            ...
            return (
              <div key={catKey} className="mb-16 last:mb-0">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="font-display text-2xl font-semibold text-navy">{meta.label}</h2>
                </div>

                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {items.map((p: any) => (
                    <div key={p.id} className="rounded-2xl bg-white border border-line p-6 shadow-sm hover:border-gold/50 hover:shadow-md transition-all flex flex-col">
                      <h3 className="font-display font-semibold text-navy mb-2">{p.title}</h3>
                      <p className="text-sm text-slate flex-1">{p.description}</p>
                      <div className="flex items-center justify-between mt-6">
                        <span className="font-display text-lg font-semibold text-gold">
                          ₦{Number(p.price).toLocaleString()}
                        </span>
                        <AddToCartButton id={p.id} title={p.title} price={Number(p.price)} type={p.type} />
                      </div>
                    </div>
                  ))}
                </div>
